    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            if self.config.agent.activate_agent:
                ...
                # Note: agent_rollout_loop already handles interleaving for all non_tensor_batch data (including extra_info)
                if self.sampling_params.n > 1 and do_sample:
                    idx = _repeat_interleave(idx, self.sampling_params.n)
                    attention_mask = _repeat_interleave(attention_mask, self.sampling_params.n)
                    position_ids = _repeat_interleave(position_ids, self.sampling_params.n)
                    batch_size = batch_size * self.sampling_params.n
                print(f' [DEBUG agent mode] After repeat: idx.shape={idx.shape}, response.shape={response.shape}, batch_size={batch_size}')
            else:
            ...
            if self.sampling_params.n > 1 and do_sample and not self.config.agent.activate_agent:
                ...
                # 同样需要 repeat extra_info 以匹配新的 batch_size
        ...
        if self.config.agent.activate_agent:
            ...
            for key, val in non_tensor_batch.items():
                if isinstance(val, np.ndarray):
                    print(f' [DEBUG agent non_tensor] {key}: shape={val.shape}, dtype={val.dtype}')
                    if key == 'conversation_history' and len(val) > 0:
                        print(f' [DEBUG agent non_tensor] conversation_history[0]: {val[0]}')
                    if key == 'original_images' and len(val) > 0:
                        print(f' [DEBUG agent non_tensor] original_images[0] is None: {val[0] is None}')
