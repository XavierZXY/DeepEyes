...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    for step in range(config.agent.max_turns):
        ...
        for idx, a in enumerate(actions):
            print('[Debug] action {idx}:', a.outputs[0].text)
...
def execute_tool_call(sample, tokenizer=None, processor=None, pbar=None):
    from .envs.mm_process_engine.tool_adapter import execute_new_format_tool_call
    ...
    # 尝试使用新格式的工具调用适配器
    try:
        tool_result, reward, done, info = execute_new_format_tool_call(tool, action_string)
    except Exception as e:
        print(f"[DEBUG] New format failed, falling back to legacy format: {e}")
        # 如果新格式失败，回退到旧格式
...
class ParallelEnv:
    ...
    def step(self, active_indices, actions):
        ...
        if num_workers <= 1:
            for agi in agent_inputs:
                print('-'*50)
                print(f' [DEBUG action] {agi["action"]} 9999')
                print(f' [DEBUG tool] {type(agi["tool"])} 666')
                print(f' [DEBUG tool name] {agi["tool"].name if agi["tool"] is not None else None} 666')
                print(f' [DEBUG tool desc] {agi} 666')
                subidx = agi['idx']
    ...
    def reset(self, prompts, vllm_inputs, n=1, **kwargs):
        ...
        for i in range(len(prompts)):
            ...
            print(data_item.non_tensor_batch.keys(),99999)
