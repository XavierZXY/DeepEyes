...
class MMSearchEngine(ToolBase):
    ...
    def execute(self, action_string, **kwargs):
        ...
        search_list = extract_tool_call_contents(
            self.search_start, self.search_end, action_string
        )
        browse_list = extract_tool_call_contents(
            self.browse_start, self.browse_end, action_string
        )
