        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            if config is not None:
                ...
                try:
                    ...
                    print(f"Saved config file to wandb: {config_file_path}")
                except Exception as e:
                    print(f"Warning: Failed to save config file to wandb: {e}")
                finally:
                    import os
                    if os.path.exists(config_file_path):
                        os.remove(config_file_path)
            
            # Also save trainer_config as artifact for better organization
            if trainer_config is not None:
                try:
                    ...
                    # Create a directory for config artifacts
                    config_dir = os.path.join(trainer_config.trainer.default_local_dir, "wandb_config")
                    os.makedirs(config_dir, exist_ok=True)
                    
                    config_file = os.path.join(config_dir, f"{experiment_name}_config.yaml")
                    
                    # Save the full config to a yaml file
                    if hasattr(trainer_config, '__dict__'):
                        config_dict = OmegaConf.to_container(trainer_config, resolve=True)
                    else:
                        config_dict = config
                    ...
                    # Create and log wandb artifact for config
                    artifact = wandb.Artifact(
                        name=f"config-{experiment_name}",
                        type="config",
                        description=f"Training configuration for {experiment_name}"
                    )
                    artifact.add_file(config_file)
                    wandb.log_artifact(artifact)
                    
                    print(f"Saved full config to: {config_file}")
                    print(f"Logged config artifact to wandb")
                    
                    # Save and upload key source code files
                    code_dir = os.path.join(config_dir, "source_code")
                    os.makedirs(code_dir, exist_ok=True)
                    
                    # Define key source files to upload
                    key_source_files = [
                        "verl/utils/reward_score/image_restoration.py",
                        "verl/workers/agent/parallel_env.py",
                    ]
                    
                    # Try to find project root (where verl/ directory is located)
                    current_file = os.path.abspath(__file__)  # tracking.py
                    verl_utils_dir = os.path.dirname(current_file)  # verl/utils/
                    verl_dir = os.path.dirname(verl_utils_dir)  # verl/
                    project_root = os.path.dirname(verl_dir)  # project root
                    
                    # Create code artifact (use 'source_code' instead of reserved 'code' type)
                    code_artifact = wandb.Artifact(
                        name=f"source-code-{experiment_name}",
                        type="source_code",
                        description=f"Key source code files for {experiment_name}"
                    )
                    ...
                    for rel_path in key_source_files:
                        ...
                        if os.path.exists(src_file):
                            ...
                            # Add to artifact with relative path
                            code_artifact.add_file(dst_file, name=rel_path)
                            saved_files.append(rel_path)
                            print(f"Saved source file: {rel_path}")
                        else:
                            print(f"Warning: Source file not found: {src_file}")
                    
                    if saved_files:
                        wandb.log_artifact(code_artifact)
                        print(f"Logged {len(saved_files)} source code files to wandb")
                    
                except Exception as e:
                    print(f"Warning: Failed to save config/code artifacts: {e}")
                    import traceback
                    traceback.print_exc()
