...
def check_multiturn_format_v2(response_str: str) -> float:
    """
    Check multi-turn conversation format:
    1. Each turn must have <think> block
    2. Each turn can have either <tool_call> OR <answer>, not both
    3. Final turn must have <answer>
    4. All JSON formats must be valid
    5. All tool names must be allowed
    ...
    if not think_matches:
        return 0.0  # No think blocks found
    ...
    for i, turn in enumerate(turns):
        ...
        # 5. Final turn must have answer
        if is_final_turn and not answer_match:
            return 0.0
        ...
        # 7. Validate answer format if present
        if answer_match:
            try:
                answer_json = json.loads(answer_match.group(1))
...
def check_response_format_strict_v2(response_str: str, content_aware: bool = False) -> float:
    ...
    3. 必须有<tool_call>或<answer>中的一个
    4. <answer>必须包含valid JSON with restoration_log (only field)
    5. tool_call中的工具名称必须是system prompt中允许的工具
    
    Note: 在多轮对话中，中间可以有tool_call回合，但最终必须以answer回合结束
...
def compute_image_quality_reward_v2(solution_str: str, extra_info: Dict = None) -> float:
    ...
    try:
        ...
        if original_image is None or restored_image is None:
            return return_negative_quality_result("Failed to extract images from multimodal data")
...
def compute_score_v2(solution_str: str, ground_truth: Union[str, Dict], extra_info: Dict = None, 
                     ...
                     format_content_aware: bool = False) -> float:
    """
    ...
    if strict_format:
        # Use multi-turn format checking for strict mode
    ...
    if is_clean_sample:
        ...
        if check_clean_image_response_v2(solution_str):
            accuracy_score = 1.0
            print(f' [DEBUG clean_sample] Correctly identified clean image, accuracy_score=1.0')
        else:
            accuracy_score = -1.0
    ...
    if is_clean_sample:
        print(f' [DEBUG image_restoration_v2] clean sample detected, using accuracy reward instead of image quality')
