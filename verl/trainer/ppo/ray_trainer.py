...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        # Save initial state of val_dataloader before any iteration
        self._val_dataloader_initial_state = self.val_dataloader.state_dict()
        print(f"[DEBUG] Saved initial val_dataloader state")
    ...
    def _validate(self):
        # Reset val_dataloader state to ensure we iterate from the beginning
        # StatefulDataLoader maintains iterator state, need to reset before each validation
        self.val_dataloader.load_state_dict(self._val_dataloader_initial_state)
        print(f"[DEBUG VAL DATALOADER] Step {self.global_steps}: Reset val_dataloader to initial state")
        ...
        # Lists to collect data for image logging
        val_image_histories = []
        val_raw_prompts = []
        val_responses = []
        val_original_images = []
        ...
        for batch_idx, test_data in enumerate(self.val_dataloader):
            ...
            if "multi_modal_inputs" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    ...
                    non_tensor_batch_keys=['raw_prompt_ids', 'multi_modal_data', 'origin_multi_modal_data', 'multi_modal_inputs', 'extra_info'],  # 添加extra_info
                )
            else:
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", 'extra_info'],  # 添加extra_info
                )
            ...
            if "reward_extra_info" in result:
                for key, lst in result["reward_extra_info"].items():
                    ...
                    # Debug: print quality scores for validation
                    if key in ['ir_accuracy_score', 'accuracy_score', 'image_quality_reward']:
                        print(f"[DEBUG VAL] Step {self.global_steps}: {key} = {lst[:5] if len(lst) > 5 else lst}")

            data_source_lst.append(test_batch.non_tensor_batch.get("data_source", ["unknown"] * reward_tensor.shape[0]))
            
            # Collect data for image logging
            # Note: agent mode uses 'image_history_list', non-agent mode uses 'image_history'
            image_history_key = 'image_history_list' if 'image_history_list' in test_batch.non_tensor_batch else 'image_history'
            if image_history_key in test_batch.non_tensor_batch:
                image_histories = test_batch.non_tensor_batch[image_history_key]
                if isinstance(image_histories, np.ndarray):
                    val_image_histories.extend(image_histories.tolist())
                elif isinstance(image_histories, list):
                    val_image_histories.extend(image_histories)
                print(f"[DEBUG VAL IMAGE] Collected {len(image_histories)} image histories using key '{image_history_key}'")
            
            # Collect original images
            if 'original_images' in test_batch.non_tensor_batch:
                original_imgs = test_batch.non_tensor_batch['original_images']
                if isinstance(original_imgs, np.ndarray):
                    val_original_images.extend(original_imgs.tolist())
                elif isinstance(original_imgs, list):
                    val_original_images.extend(original_imgs)
                print(f"[DEBUG VAL IMAGE] Collected {len(original_imgs)} original images")
            
            # Collect conversation histories
            if 'conversation_history' in test_batch.non_tensor_batch:
                conv_hists = test_batch.non_tensor_batch['conversation_history']
                if isinstance(conv_hists, np.ndarray):
                    val_conversation_histories.extend(conv_hists.tolist())
                elif isinstance(conv_hists, list):
                    val_conversation_histories.extend(conv_hists)
                print(f"[DEBUG VAL IMAGE] Collected {len(conv_hists)} conversation histories")
        ...
        print(f"[DEBUG VAL DATALOADER] Step {self.global_steps}: reward_extra_infos_dict keys: {list(reward_extra_infos_dict.keys())}")
        ...
        if hasattr(self, 'logger') and 'wandb' in self.logger.logger and self.config.trainer.get('log_images_to_wandb', True):
            if len(val_image_histories) > 0:
                try:
                    ...
                    system_prompt = None
                    try:
                        if len(val_raw_prompts) > 0 and isinstance(val_raw_prompts[0], list):
                            for msg in val_raw_prompts[0]:
                                if isinstance(msg, dict) and msg.get('role') == 'system':
                                    system_prompt = msg.get('content', '')
                                    break
                    except Exception as e:
                        print(f"[DEBUG] Could not extract system prompt: {e}")
    ...
    def fit(self):
        ...
        # Store logger as instance variable for image logging
        self.logger = logger
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'raw_prompt' in batch.non_tensor_batch.keys():
                    ...
                    # print(' [DEBUG raw prompt] raw_prompt pop into gen_batch')  # 注释掉，避免打印大量image_pad
                ...
                if reward_extra_infos_dict:
                    ...
                    print(f"[DEBUG REWARD METRICS] Collected {len(reward_component_metrics)} reward component metrics")

                # Upload rollout trajectory images to wandb (training mode)
                if 'wandb' in logger.logger and self.config.trainer.get('log_images_to_wandb', True):
                    try:
                        # Extract image quality scores from reward_extra_infos_dict
                        image_quality_scores = extract_image_quality_scores_from_rewards(reward_extra_infos_dict)
                        ...
                        print(f"[DEBUG WANDB IMAGE] Using key '{image_history_key}' for image history")
                        print(f"[DEBUG WANDB IMAGE] Found {len(batch_data['image_history'])} image histories")
                        print(f"[DEBUG WANDB IMAGE] Found {len(batch_data['original_images'])} original images")
                        ...
                        try:
                            ...
                            if len(raw_prompts) > 0 and isinstance(raw_prompts[0], list):
                        ...
                        # Log images with strategic sampling
                        log_rollout_images_to_wandb(
                            wandb_logger=logger.logger['wandb'],
                            batch_data=batch_data,
                            image_quality_scores=image_quality_scores,
                            detailed_metrics=detailed_metrics,
                            step=self.global_steps,
                            mode='train',
                            num_samples_train=self.config.trainer.get('num_train_images_to_log', 5),
                            num_best_worst=self.config.trainer.get('num_best_worst_images_to_log', 2),
                            tokenizer=self.tokenizer,
                            system_prompt=system_prompt,
                        )
                        
                        # 保存markdown文件（本地查看）
                        if self.config.trainer.get('save_conversation_markdown', False):
                            from verl.utils.tracking_image_utils import save_conversations_to_markdown
                            save_conversations_to_markdown(
                                batch_data=batch_data,
                                image_quality_scores=image_quality_scores,
                                detailed_metrics=detailed_metrics,
                                step=self.global_steps,
                                mode='train',
                                tokenizer=self.tokenizer,
                                output_dir=f"outputs/conversations/{self.config.trainer.experiment_name}"
                            )
                    except Exception as e:
